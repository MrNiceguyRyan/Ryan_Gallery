 ...
 *     --ids=collectionId1,collectionId2
 *                                           (delete only named empty collections)
 *
 ...
 * explicit collection ID allowlist. It only touches allowlisted collections
 * with exactly 0 referencing photos.
 */
...
const ID_ARG = process.argv.find((arg) => arg.startsWith('--ids='));
const DELETE_IDS = new Set(
  (ID_ARG?.slice('--ids='.length) || '').split(',').map((id) => id.trim()).filter(Boolean),
);
...
if (DELETE_EMPTY && DELETE_IDS.size === 0) {
  console.error('Refusing to delete every empty collection. Pass an explicit --ids=<collectionId,...> allowlist.');
  process.exit(1);
}
...
async function main() {
  ...
  if (!APPLY) {
    console.log('   Re-run with --apply to trim names; add --delete-empty --ids=<collectionId,...> to remove selected empties.\n');
    return;
  }
  ...
  if (DELETE_EMPTY) {
    for (const c of empties.filter((collection) => DELETE_IDS.has(collection._id))) {
      await sanity.delete(c._id);
      console.log(`   ✓ deleted empty collection "${c.name}" (${c._id})`);
    }
  } else if (empties.length) {
    console.log('   (empties left in place — add --delete-empty --ids=<collectionId,...> to remove selected entries)');
  }
  console.log('\n   ✅ Done.\n');
}
