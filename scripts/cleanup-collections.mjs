 ...
 *     --ids=<collectionId,...>                        (delete only the listed
 ...
 * explicit collection ID allowlist. It only touches listed collections with
...
if (!SANITY_TOKEN) {
  console.error('SANITY_TOKEN is required to clean up collections.');
  process.exit(1);
}

if (DELETE_EMPTY && DELETE_IDS.size === 0) {
  console.error('--delete-empty requires --ids=<collectionId,...>. No collections were deleted.');
  process.exit(1);
}
...
async function main() {
  ...
  if (DELETE_EMPTY) {
    for (const c of empties) {
      if (!DELETE_IDS.has(c._id)) continue;
      await sanity.delete(c._id);
      console.log(`   ✓ deleted empty collection "${c.name}" (${c._id})`);
    }
  } else if (empties.length) {
    console.log('   (empties left in place — add --delete-empty --ids=<collectionId,...> to remove selected collections)');
  }
  console.log('\n   ✅ Done.\n');
}
