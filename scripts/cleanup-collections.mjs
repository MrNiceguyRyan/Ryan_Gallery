...
if (!SANITY_TOKEN) {
  throw new Error('SANITY_TOKEN is required to clean up collections.');
}
...
async function main() {
  ...
  for (const c of cols) {
    ...
    console.log(`   ${String(c.n).padStart(3)} photos · ${(c.region || '—').padEnd(12)} · "${c.name}" (${c._id})${flags.length ? '   ⟶ ' + flags.join(', ') : ''}`);
  }

  console.log(`\n   ${trims.length} name(s) to trim · ${empties.length} empty collection(s)\n`);

  if (!APPLY) {
    console.log('   Re-run with --apply to trim names; add --delete-empty --ids=<id,id> to remove selected empties.\n');
    return;
  }
  ...
  if (DELETE_EMPTY && DELETE_IDS.size === 0) {
    console.log('   Refusing to delete empties without an explicit --ids=<collectionId,...> list.');
    ...
    console.log('   (empties left in place — add --delete-empty --ids=<id,id> to remove selected empties)');
  }
  console.log('\n   ✅ Done.\n');
}
