 ...
 *
 * Requires: SANITY_TOKEN env var for writes.
...
if (!SANITY_TOKEN) {
  console.error('Missing SANITY_TOKEN. Set it in the environment before running upload-photos.mjs.');
  process.exit(1);
}
...
async function uploadState(stateName) {
  ...
  if (cities.length === 0) {
    ...
    if (directPhotos.length > 0) {
      ...
      // Treat state as the collection name while reading files from the state folder.
