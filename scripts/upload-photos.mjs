...
if (!SANITY_TOKEN) {
  console.error('Missing SANITY_TOKEN. Set it to a Sanity write token before running this upload script.');
  process.exit(1);
}
...
async function uploadCity(stateName, cityFolderName) {
  const cityDisplayName = cityFolderName === '.' ? stateName : cityFolderName;
  const cityPath = cityFolderName === '.'
    ? join(PHOTO_ROOT, stateName)
    : join(PHOTO_ROOT, stateName, cityFolderName);
  const locationKey = cityDisplayName.toLowerCase().trim();
  ...
  if (files.length === 0) {
    console.log(`    ⚠️  No images in ${cityDisplayName}, skipping.`);
    return;
  }
  console.log(`    📷 ${cityDisplayName}: ${files.length} images`);

  // Find or create collection
  const collectionId = await findOrCreateCollection(cityDisplayName, stateName);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      // Generate title
      const title = await generateTitle(cityDisplayName, stateName, existingPhotos.length + i, exif);
