...
if (!SANITY_TOKEN) {
  console.error('Missing SANITY_TOKEN. Set it in your environment before uploading photos.');
  process.exit(1);
}
...
async function uploadState(stateName) {
  ...
  if (cities.length === 0) {
    ...
    if (directPhotos.length > 0) {
      ...
      // Treat the state folder as the city/collection so flat uploads (e.g. Macau)
      // do not create a "." collection with missing location metadata.
