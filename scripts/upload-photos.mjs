...
async function findOrCreateCollection(cityFolderName, stateName) {
  ...
  if (!slug) {
    throw new Error(`Cannot create a collection with an empty slug from folder "${cityFolderName}"`);
  }
...
async function uploadState(stateName) {
  ...
  if (cities.length === 0) {
    ...
    if (directPhotos.length > 0) {
      ...
      // Treat the state folder as its own collection while reading photos
      // from the state directory itself.
