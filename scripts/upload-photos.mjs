...
if (!SANITY_TOKEN) {
  console.error('Missing SANITY_TOKEN. Set it in the environment before uploading photos.');
  process.exit(1);
}
...
async function uploadState(stateName) {
  ...
  if (cities.length === 0) {
    ...
    if (directPhotos.length > 0) {
      ...
      // Treat the state folder itself as the collection so flat folders like
      // Macau do not create a "." collection with missing location metadata.
