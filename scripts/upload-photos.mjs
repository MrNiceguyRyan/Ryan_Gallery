...
if (!SANITY_TOKEN) {
  console.error('Missing SANITY_TOKEN env var. Create a write token in Sanity and run SANITY_TOKEN=... node scripts/upload-photos.mjs');
  process.exit(1);
}
...
async function uploadState(stateName) {
  ...
  if (cities.length === 0) {
    ...
    if (directPhotos.length > 0) {
      ...
      // Treat the state folder itself as the collection instead of creating a
      // corrupt "." collection when there is no city subfolder.
